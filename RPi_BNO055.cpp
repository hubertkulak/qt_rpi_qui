...
void Adafruit_BNO055::savevector()
{
  //int8_t temp = getTemp();
  //std::cout << "Current Temperature from first sensor: "<< (int)temp << " C" << std::endl;
...
void Adafruit_BNO055::printvector()
{
    std::cout << "x: " << myStruct.x;
    std::cout << " y: " << myStruct.y;
    std::cout << " z: " << myStruct.z << std::endl;
}
...
bool Adafruit_BNO055::imu_initialization()
{
  ...
  _HandleBNO=i2cOpen(_i2cChannel,BNO055_ADDRESS_B,0);
  //bno1._HandleBNO=i2cOpen(bno1._i2cChannel,BNO055_ADDRESS_B,0);
...
void Adafruit_BNO055::sig_bno_handler(){


     std::cout<<"First sensor data read:"<<std::endl;
     savevector();
     printvector();

     gpioSleep(PI_TIME_RELATIVE, 1, 10000);

    // std::cout<<"Second sensor data read:"<<std::endl;
     //bno1.savevector();
     //bno1.printvector();
